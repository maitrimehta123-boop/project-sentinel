...
export const handler = async (event: FunctionEvent) => {
  ...
  try {
    ...
    if (
      ...
      !Number.isSafeInteger(expectedPaise) ||
      expectedPaise < MIN_PAYMENT_PAISE
    ) {
      return json(400, { error: "Invalid payment confirmation." });
    }

    // Never trust the order id supplied by the browser for signature verification.
    ...
    if (
      ...
      return json(400, { error: "Payment details do not match this order." });
    }

    // Some Razorpay accounts can return "authorized" before capture.
    // Capture server-side so a webhook is not required for the normal checkout flow.
    if (payment.status === "authorized") {
      try {
        ...
        // Re-fetch once because another process/account auto-capture may have completed it.
    ...
    if (
      ...
      await admin.from("orders").update({
        status: "PAYMENT_AUTHENTICATED",
        payment_status: "authenticated",
      }).eq("id", order.id);

      return json(409, {
        error: "Payment was received but is not captured yet. Please do not pay again.",
    ...
    console.error(
      "verify_razorpay_payment_error",
      error instanceof Error ? error.message : "unknown",
    );
    return json(502, {
      error: "Could not verify the payment. Please contact support if money was debited.",
    });
  }
};
