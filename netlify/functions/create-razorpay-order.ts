  ...
  try {
    ...
    for (const item of body.items) {
      ...
      if (dbProduct && !dbProduct.active) {
        return json(400, { error: "One of the items is no longer available." });
      }
    ...
    if (!Number.isSafeInteger(amountPaise) || amountPaise < MIN_PAYMENT_PAISE) {
      return json(400, { error: "Order amount is invalid." });
    }
    ...
    try {
      ...
      if (
        gatewayOrder.amount !== amountPaise ||
